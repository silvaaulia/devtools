...
function convertToXML() {
    ...
    try {
        ...
        if (lines.length < 2) { showMessage('CSV needs header and data', 'error'); return; }
        ...
        lines.slice(1).forEach(line => {
            ...
            xml += '  <item>\n';
            headers.forEach((h, i) => {
                const safeTag = h.trim().replace(/[^a-zA-Z0-9_]/g, '_');
                xml += `    <${safeTag}>${escapeXml(values[i]?.trim() || '')}</${safeTag}>\n`;
            });
            xml += '  </item>\n';
        });
        xml += '</root>';
